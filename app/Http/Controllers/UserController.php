...
class UserController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try { 
            ...
            if ($existingUser) {
                return redirect('users/add-user')->withErrors([
            ...
            return redirect('users/add-user')->with('status', 'Error: ' . $e->getMessage())->withInput();
        }
    ...
    public function edit($id){
        ...
        if($editdata){
            return view('users.edit', compact('editdata', 'key'));
        } else {
            return redirect('/users')->with('status', 'User ID Not Found');
        }
    }
    ...
    public function update(Request $request, $id) {
        ...
        if (!$existingUser || !isset($existingUser['firebase_uid'])) {
            return redirect('/users')->with('status', 'User ID or Firebase UID not found.');
        }
        ...
        if ($existingUser['first_name'] . ' ' . $existingUser['last_name'] !== $validatedData['first_name'] . ' ' . $validatedData['last_name']) {
            $changes[] = "name from '{$existingUser['first_name']} {$existingUser['last_name']}' to '{$validatedData['first_name']} {$validatedData['last_name']}'";
        }
        ...
        try {
            ...
            return redirect('/users')->with('status', 'Error: ' . $e->getMessage())->withInput();
        }
    }
        ...
        if ($archive_data) {
            ...
            if ($delete_data) {
                try {
                    ...
                    return redirect('/users')->with('status', 'User Archived Successfully');
                } catch (\Exception $e) {
                    return redirect('/users')->with('status', 'User Archived but Failed to Remove from Authentication: ' . $e->getMessage());
                }
            } else {
                return redirect('/users')->with('status', 'User Not Archived');
            }
        } else {
            return redirect('/users')->with('status', 'User Not Archived');
        }
