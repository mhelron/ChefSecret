...
class LoginController extends Controller
{
    ...
    public function showLoginForm()
    {
        if (Session::has('firebase_id_token')) {
            try {
                ...
                return redirect()->route('dashboard.index')->with('status', 'You are already logged in.');
            } catch (\Exception $e) {
                Session::flush();
            }
        }
    ...
    protected function roleBasedRedirect($userRole)
    {
        switch ($userRole) {
            case 'Super Admin':
                return redirect('/dashboard')->with('status', 'Logged In as ' . session('firstname') . ' (Super Admin)');
            case 'Admin':
                return redirect('/dashboard')->with('status', 'Logged In as ' . session('firstname') . ' (Admin)');
            case 'Manager':
                return redirect('/dashboard')->with('status', 'Logged In as ' . session('firstname') . ' (Manager)');
            case 'Staff':
                return redirect('/dashboard')->with('status', 'Logged In as ' . session('firstname') . ' (Staff)');
            default:
    ...
    public function logout()
    {
        ...
        Session::flush();
        return redirect()->route('login.index')->with('status', 'Logged out successfully!');
    }
